...
public class Seccion {
	...
	public HashMap<Posicion, Ventana> ventanas; 
	int roturasRestantes;
	Random randomizador;
	...
	/**
	 * Crea y retorna un <kbd>array[5][3]</kbd> auxiliar con valores tipo <kbd>String</kbd> para la contruccion del <kbd>HashMap</kbd>.<br>
	 * Primero lo inicializa con el valor "Normal" en cada casilla (tipo mas comund de ventana).<br>
	 * Luego repite el siguiente proceso hasta que no haya mas <i>Hojas</i> que colocar:<br>
	 * - Elije aleatoriamente una casilla.<br>
	 * - Se asegura de que no haya sido reemplazada previamente y que no sea la posicion de una ventana
	 * semicircular (Que seimpre se encuentran en los casilleros [3][2] y [3][3].<br>
	 * - La reemplaza por el valor "Hoja".

	 * @param hojasRestantes Cantidad de ventanas tipo {@link Hoja} a colocar en el {@link HashMap}
	 * @param esPrimeraSeccion Boolean utilizado para el chequeo de ventanas tipo {@link Semicircular}
	 * @return <kbd>array[5][3]</kbd> de tipo String con los tipos de ventana a crear en la seccion.
	 * @see Seccion#Seccion(boolean, int, int)
	 */
	private String[][] randomizarTipos(int hojasRestantes, boolean esPrimeraSeccion) {
		String[][] matrizDeTipos = new String[(Constantes.VENTANASPORFILA)][(Constantes.CANTIDADFILAS)];

		for (int i = 0; i < Constantes.CANTIDADFILAS; i++) {
			for (int j = 0; j < Constantes.VENTANASPORFILA; j++) {
				matrizDeTipos[j][i] = "Normal";
			}
		}
		while (hojasRestantes > 0) {
			int filaElejida = randomizador.nextInt(Constantes.VENTANASPORFILA);
	...
	/**
	 * Crea y retorna un <kbd>array[5][3]</kbd> auxiliar con valores tipo <i>int</i>, que representan la
	 * cantidad obstaculos verticales (<i>molduras</i> y <i>maceteros</i>) que posee la seccion actual.<br>
	 * Primero lo inicializa con el valor <i>0</i> en cada casilla.
	 * Luego repite el siguiente proceso hasta que no haya mas <i>obstaculos</i> que colocar:<br>
	 * - Elije aleatoriamente una casilla.<br>
	 * - Se asegura de que no haya mas de 2 obstaculos por ventana (solo puede haber 1 moldura, 1 macetero o ambos)
	 * - Incrementa el valor en 1.
	 * 
	 * @param obstaculosRestantes cantidad de obstaculos verticales a colocar.
	 * @param esPrimeraSeccion Boolean utilizado para el chequeo de ventanas tipo {@link Semicircular}
	 * @return <kbd>array[5][3]</kbd> de tipo <i>int</i> con la cantidad obstaculos verticales a colocar en la seccion.
	 * @see Seccion#Seccion(boolean, int, int)
	 */
	private int[][] randomizarObstaculos(int obstaculosRestantes, boolean esPrimeraSeccion) {
		int[][] matrizDeObstaculos = new int[(Constantes.VENTANASPORFILA)][(Constantes.CANTIDADFILAS)];
	...
	/**
	 * Utilizando las matrices axiliares recibidas como parametro, crea y retorna un <kbd>HashMap[Posicion,Ventana]</kbd> inicializando
	 * cada ventana con sus correctas características (tipo, y si tiene o no moludar y/o macetero), 
	 * sin ningunra rotura y asegurandose de mantener el orden logico 3 filas, 5 ventanas por fila.
	 * @param matrizDeTipos
	 * @param matrizDeObstaculos
	 * @return HashMap[Posicion,Ventana] con las ventanas inicializadas y sin roturas.
	 * @see Seccion#Seccion(boolean, int, int)
	 */
	private HashMap<Posicion, Ventana> inicializarVentanas(String[][] matrizDeTipos, int[][] matrizDeObstaculos) {
		Posicion key = null;
		Ventana value = null;
		HashMap<Posicion, Ventana> inicializarVentanas = new HashMap<Posicion, Ventana>(Constantes.VENTANASPORFILA * Constantes.CANTIDADFILAS);
	...
	/**
	 * Rompe aleatoriamente las ventanas del atributo {@link #ventanas}
	 * (previamente inicializado en {@link #inicializarVentanas(String[][], int[][]) inicializarVentanas()})
	 * la cantidad de veces indicada en el parametro <kbd>#cantidadARomper</kbd><br><br>
	 * La lógica para la randomizacion es la siguiente: <br>
	 * - Crea un <i>HashMap</i> auxiliar (<kbd>roturasPorVentana</kbd>), con las <i>keys</i><br>
	 * correspondientes al atributo <kbd>#ventanas</kbd> pero con <i>values</i> de tipo <kbd>int</kbd> para llevar el
	 * conteo de roturas en cada ventana y asegurarse de que no se rompan demas.<br>
	 * - Remueve las posiciones correspondientes a ventanas de tipo Hoja ya que las misma no pueden romperse.<br>
	 * - Extrae un ArrayList de keys del HashMap axuliar para randomizar la iteracion utilizando un objeto
	 * tipo {@link Random}.<br>
	 * - Repite los siguientes 2 pasos la cantidad de veces inticada en el parametro <i>cantidadARomper</i><br>
	 * - Elije aleatoriamente (iterando sobre el HashMap auxiliar) una ventana y la rompe 1 vez<br>
	 * - Si la ventana ahora esta completamente rota, la remueve del HashMap auxiliar<br>
	 * 
	...
	private void randomizarRoturas(int cantidadARomper) {
		...
		for (int i = 0; i < Constantes.CANTIDADFILAS; i++) {
			for (int j = 0; j < Constantes.VENTANASPORFILA; j++) {
				...
				// Evita agregar referencias a las ventanas tipo Hoja ya que las misma no se pueden romper
		...
		if (!(roturasPorVentana.isEmpty())) {
			...
			while ((cantidadARomper > 0) && (keysAsArray.size() > 0)) {
				...
				cantidadARomper--;
				// Remueve del set si esta completamente rota para no ser re-elejida en la randomizacion
	 ...
	 * Se cambia el estado de un único {@link Panel} aleatorio de:
	 ...
	 */
	Posicion nuevaPosicion(Posicion posicionActual, Direccion direccion) {
	...
	Posicion hayAbierta() {
		...
		if (!(tempArray.isEmpty())) {
			...
			for (int i = 0; i < iteraciones ; i++) {
				pos = new Posicion(iterador.next());
			}
		}
		return pos;
	}
	...
	public Ventana getVentana(Posicion key) {
		return ventanas.get(key);
	}
	...
	String[][] toStringArray() {
		...
		String[][] representacion = new String[6][4];
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 5; j++) {
				pos.setCoordenadaX(j*100);
				pos.setCoordenadaY(i*100);
				representacion[j][i] = ventanas.get(pos).toString();
			}
		}
